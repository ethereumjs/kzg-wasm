...
describe('api tests', () => {
    ...
    it('should generate kzg commitments and verify proofs', () => {
        kzg.loadTrustedSetup()
        const blob = new Uint8Array(BYTES_PER_BLOB)
        blob[0] = 0x01
        blob[1] = 0x02
        const commitment = kzg.blobToKzgCommit(blob)
        assert.equal(commitment.toLowerCase(), 'ab87358a111c3cd9da8aadf4b414e9f6be5ac83d923fb70d8d27fef1e2690b4cad015b23b8c058881da78a05c62b1173')
        ...
        const proofVerified = kzg.verifyBlobKzgProof(blob, hexToBytes('0x' + commitment), hexToBytes('0x' + proof))
        assert.equal(proofVerified, 0)
        kzg.freeTrustedSetup()
    })
    it('should verify kzg proofs with points', () => {
        kzg.loadTrustedSetup()
